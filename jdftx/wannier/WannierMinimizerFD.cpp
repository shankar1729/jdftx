...
inline void sumRuleCEDA(double weight, const vector3<>& b,
	const diagMatrix& E1, const diagMatrix& E2,
	const diagMatrix& F1, const diagMatrix& F2,
	const matrix& M12, diagMatrix& num, diagMatrix& den)
{
	double twoByBsq = 2./b.length_squared();
	double numContrib = 0., denContrib = 0.; //cumulative contributions to numerator and denominator
	for(int j=0; j<E1.nRows(); j++)
	{	//Terms with single sum over bands:
		numContrib += F1[j] * (1. + twoByBsq * E1[j]);
		denContrib += F1[j] * twoByBsq;
		//Terms with double sum over bands:
		//--- contribution at current j
		for(int i=0; i<=j; i++)
		{	double term = F1[j] * twoByBsq * M12(j,i).norm();
			numContrib -= term * ((1.-F2[i])*E2[i] + F2[i]*E1[j]);
			denContrib -= term;
		}
		//--- contribution at previous j and at i=j
		for(int jPrev=0; jPrev<j; jPrev++)
		{	int i = j;
			double term = F1[jPrev] * twoByBsq * M12(jPrev,i).norm();
			numContrib -= term * ((1.-F2[i])*E2[i] + F2[i]*E1[jPrev]);
			denContrib -= term;
		}
		//Accumulate to num and den with appropriate band count:
		num[j] += weight * numContrib;
		den[j] += weight * denContrib;
	}
}

void WannierMinimizerFD::saveExtra(int iSpin)
{
	//Output CEDA energies:
	if(wannier.ceda)
	{
		//Get chemical potential:
		int nBandsMin = (int)ceil(e.eInfo.nElectrons/e.eInfo.qWeightSum);
		if(nBands <= nBandsMin) die("No empty states found: cannot compute CEDA energy.\n");
		double mu = e.eInfo.findMu(e.eVars.Hsub_eigs, e.eInfo.nElectrons);
		
		//Calculate / collect band structure and fillings for the full k-mesh:
		std::vector<diagMatrix> eigsB(kMesh.size(), eye(nBands)), eigsW(kMesh.size(), eye(nCenters));
		std::vector<diagMatrix> F_B(kMesh.size(), eye(nBands)), F_W(kMesh.size(), eye(nCenters));
		std::vector<matrix> BtoW(kMesh.size(), zeroes(nBands, nCenters)); //matrices that transform Bloch states to Wannier states at each k
		for(size_t ik=0; ik<kMesh.size(); ik++) if(isMine_q(ik, iSpin))
		{	eigsB[ik] = e.eVars.Hsub_eigs[kMesh[ik].point.iReduced + iSpin*qCount];
			//Wannier Hamiltonian at k:
			matrix Hsub = dagger(kMesh[ik].U) * eigsB[ik] * kMesh[ik].U, evecsW;
			Hsub.diagonalize(evecsW, eigsW[ik]);
			BtoW[ik] = kMesh[ik].U * evecsW;
			//Calculate fillings
			F_B[ik] = e.eInfo.fermi(mu, eigsB[ik]);
			F_W[ik] = e.eInfo.fermi(mu, eigsW[ik]);
		}
		...
		//Evaluate plasma-frequency sum rule for neighbours in Wannier FD formula:
		diagMatrix numB(nBands, 0.), denB(nBands, 0.); //numerator and denominator for CEDA energy with number of original Bloch bands
		diagMatrix numW(nCenters, 0.), denW(nCenters, 0.); //numerator and denominator for CEDA energy with number of Wannier-interpolated bands
		for(size_t ik1=ikStart; ik1<ikStop; ik1++)
		{	const KmeshEntry& k1 = kMesh[ik1];
			for(const Edge& edge: edges[ik1])
			{	size_t ik2 = edge.ik;
				double weight = k1.point.weight * edge.wb;
				//Bloch:
				const matrix& M_B = edge.M0;
				sumRuleCEDA(weight,  edge.b, eigsB[ik1], eigsB[ik2], F_B[ik1], F_B[ik2],        M_B,  numB, denB);
				sumRuleCEDA(weight, -edge.b, eigsB[ik2], eigsB[ik1], F_B[ik2], F_B[ik1], dagger(M_B), numB, denB);
				//Wannier:
				const matrix M_W = dagger(BtoW[ik1]) * M_B * BtoW[ik2];
				sumRuleCEDA(weight,  edge.b, eigsW[ik1], eigsW[ik2], F_W[ik1], F_W[ik2],        M_W,  numW, denW);
				sumRuleCEDA(weight, -edge.b, eigsW[ik2], eigsW[ik1], F_W[ik2], F_W[ik1], dagger(M_W), numW, denW);
			}
		}
		numB.allReduce(MPIUtil::ReduceSum);
		denB.allReduce(MPIUtil::ReduceSum);
		numW.allReduce(MPIUtil::ReduceSum);
		denW.allReduce(MPIUtil::ReduceSum);
		
		if(mpiUtil->isHead())
		{
			//Save CEDA energies for Bloch basis:
			string fname = wannier.getFilename(Wannier::FilenameInit, "CEDA", &iSpin);
			logPrintf("Dumping '%s' ... ", fname.c_str());
			FILE* fp = fopen(fname.c_str(), "w");
			for(int i=0; i<nBands; i++) fprintf(fp, "%+19.15lf\n", numB[i]/denB[i]);
			fclose(fp);
			logPrintf("done.\n"); logFlush();

			//Save CEDA energies for Wannier basis:
			fname = wannier.getFilename(Wannier::FilenameDump, "mlwfCEDA", &iSpin);
			logPrintf("Dumping '%s' ... ", fname.c_str());
			fp = fopen(fname.c_str(), "w");
			for(int i=0; i<nCenters; i++) fprintf(fp, "%+19.15lf\n", numW[i]/denW[i]);
			fclose(fp);
			logPrintf("done.\n"); logFlush();
		}
	}
