...
struct CommandDielectricResponse : public Command
{
	...
	{
		format = "[<epsilon-bulk>=80] [<Nbulk>=4.9383e-3] [<pMol>=0.92466] [<linear>=no]";
		comments =
	...
	{	const FluidSolverParams& fsp = e.eVars.fluidParams;
		logPrintf("%lg %lg %lg %s", fsp.epsilonBulk, fsp.Nbulk, fsp.pMol, boolMap.getString(fsp.linearDielectric));
	}
}
