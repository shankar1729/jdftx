...
struct CommandIonSpecies : public Command
{
	...
	{
		format = "[<path>/]<id>.<format>";
		comments =
			"Read pseudopotential from file [<path>/]<id>.<format>, which will be referred\n"
			"to internally by <id> (the basename of the file without path or extension)\n"
			"in all other commands and in the output. Currently supported <format>'s are:\n"
	...
	{	std::shared_ptr<SpeciesInfo> specie(new SpeciesInfo);
		pl.get(specie->potfilename, string(), "filename", true);
