...
public:
	...
	bool ceda; //whether to output H * P matrix elements for CEDA completion
...
private:
	...
	std::vector< std::vector< std::vector<matrix> > > dHPsub; //!< corresponding to dHsub, but with each Cartesian direction of momentum operator additionally applied (outermost index = P direction)
	...
	//optionally get the subspace Hamiltonian (and Hamiltonian * momenta) at supercell Gamma point (for all bands, not just those in nBandsOpt)
	void setSupState(std::vector<matrix>* Hsub=0, std::vector< std::vector<matrix> >* HPsub=0); 
